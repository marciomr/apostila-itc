...
Dizemos que um AFD $M = \langle Q, \Sigma, \delta, q_o, F \rangle$ {\em aceita}, ou {\em reconhece}, uma string $\omega = a_0 a_1 \dots a_n$ se existe uma sequência de estados $r_0, r_1, \dots, r_m$ tal que:
\begin{enumerate}
\item $r_0 = q_0$
...
Um AFD ao ler um símbolo $a$ em um estado $q$ tem uma única possibilidade de próximo estado (por isso determinístico).
...
Representamos o diagrama de estados da mesma forma que fizemos com os AFDs, mas agora é possível que de um mesmo estado partam mais de uma seta com o mesmo símbolo, existem setas com $\varepsilon$ e pode haver estados em que não haja seta com determinado símbolo.
...
Basta que exista um ramo, uma possibilidade de execução, para que a string seja aceita.
...
\end{displaymath}
  ...
  \begin{tabular}{c|cccc}
    $\delta'$ & $s$      & $1$           & $2$           & $f$           \\
    \hline
    $s$       & $\times$ & $\epsilon$ & $\emptyset$   & $\emptyset$   \\
    $1$       & $\times$ & $a$           & $b$           & $\emptyset$   \\
    $2$       & $\times$ & $\emptyset$   & $a \abxcup b$ & $\epsilon$ \\
